...
class TextLayer(Layer):
    def __init__(self, text, path, size, bold=False, vertical=False):
        ...
        if not vertical:
            image, bbox = self._render_hori_text(text, font, bold)
        else:
            image, bbox = self._render_vert_text(text, font, bold)
    ...
    def _get_bbox(self, text, font, vertical=False):
    ...
    def _get_inner_bbox(self, text, font, bold=False, vertical=False):
    ...
    def _render_text(self, text, font, bold=False, vertical=False):
        stroke_width = self._get_stroke_width(bold)
        direction = self._get_direction(vertical)
    ...
    def _render_hori_text(self, text, font, bold=False):
        image, bbox = self._render_text(text, font, bold)
        return image, bbox

    def _render_vert_text(self, text, font, bold=False):
        chars = utils.split_text(text, reorder=True)
        patches = []
        bboxes = []

        for char in chars:
            patch, bbox = self._render_vert_char(char, font, bold)
    ...
    def _render_vert_char(self, char, font, bold=False):
        fullwidth_char = utils.to_fullwidth(char)[0]

        if utils.vert_orient(fullwidth_char) != "Tr" and fullwidth_char.isalnum():
            return self._render_vert_upright_char(char, font, bold)

        if utils.vert_rot_flip(fullwidth_char):
            return self._render_vert_rot_flip_char(char, font, bold)

        if utils.vert_right_flip(fullwidth_char):
            return self._render_vert_right_flip_char(char, font, bold)

        if utils.vert_orient(fullwidth_char) in ("R", "Tr"):
            return self._render_vert_rot_char(char, font, bold)

        return self._render_vert_upright_char(char, font, bold)

    def _render_vert_upright_char(self, char, font, bold=False):
        vertical = len(char) <= 1
        image, bbox = self._render_text(char, font, bold, vertical)
        height, width = image.shape[:2]
        bbox = [-width // 2, 0, width, height]
        return image, bbox

    def _render_vert_rot_char(self, char, font, bold=False):
        image, bbox = self._render_text(char, font, bold)
    ...
    def _render_vert_rot_flip_char(self, char, font, bold=False):
        image, bbox = self._render_text(char, font, bold)
    ...
    def _render_vert_right_flip_char(self, char, font, bold=False):
        bbox = self._get_bbox(char, font)
        inner_bbox = self._get_inner_bbox(char, font, bold)
        sx, sy, patch_width, patch_height = inner_bbox

        patch, _ = self._render_text(char, font, bold)
